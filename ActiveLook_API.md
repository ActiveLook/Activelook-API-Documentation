...
* 4. [ActiveLook® Commands](#ActiveLookCommands)
	...
	* 4.12. [Statistics commands](#Statisticscommands)
	* 4.13. [Configuration commands](#Configurationcommands)
	* 4.14. [Deprecated commands](#Deprecatedcommands)
* 5. [ActiveLook® commands guide](#ActiveLookcommandsguide)
	...
	* 5.8. [Font](#Font)
	* 5.9. [Layout](#Layout)
		* 5.9.1. [Layout Position](#LayoutPosition)
		* 5.9.2. [Default configuration](#Defaultconfiguration)
	* 5.10. [Gauge](#Gauge)
	* 5.11. [Page](#Page)
* 6. [UI design: Good practices](#UIdesign:Goodpractices)
...
* A low energy consumption and high brightness MICROOLED® memory based micro display with a resolution of 304 x 256 pixels (16 grey levels).
...
**Command Format**: define the format of the command, including the number of bytes of the length (1 or 2 bytes) and the number of bytes of the queryID (0 to 15 bytes). Bit 5 (0bx0000) define the size of `Length`, 1: length on 2  bytes, 0 length on 1 byte. Bit 4 to 1 (0bxxxx) define the size of `Query ID` in bytes.
...
**Query ID** (optional): defines by the user, allows to identify a command (query) and the associated response. When a queryId is sent with the command, the response includes this queryId, allowing the user to match the response with the original command in an asynchronous system.
...
|--------|------------|------------------------------------------------------------------------------|
| 0x01   | Flow ON    | master can send new data                                                     |
| 0x02   | Flow OFF   | master should stop sending data                                              |
| 0x03   | Flow Error | message error, the command was incomplete or corrupt, the command is ignored |
| 0x04   | Flow Error | Receive message queue overflow                                               |
| 0x05   | Flow Error | Reserved                                                                     |
| 0x06   | Flow Error | Missing the `cfgWrite` command before configuration modification             |

⚠ The notifications should be enabled by the master device to be able to access this information.

⚠ Once the flow control OFF (`0x02`) notification is sent, the subsequent commands will be ignored until the flow control ON (`0x01`) is set.
...
| 0x0B | setName   | `str name[15]`            | 0 <= n <= 15       | Customize BLE advertising name<br>The maximum name length is 15<br>Character encoding is 'ASCII'<br>An empty name (n = 0) will reset it to the factory name |
...
| 0x11 | dim      | `u8 level`         | 1                  | Set luminance to a given percentage<br>It's not recommended to use this command when ALS is enabled since ALS will automatically overwrite dimming |
...
| 0x23 | setSensorParameters | `u8 mode`<br>`u16 alsLum[9]`     | 19              | Set sensor parameters.<br>If mode = 0, it changes the ALS array to compare when luma is changed.<br>Reboot the device is necessary |
|      |                     | `u8 mode`<br>`u16 alsPeriod`     | 3               | If mode = 1, it changes ALS period<br>Reboot of the device is necessary |
|      |                     | `u8 mode`<br>`u16 gesturePeriod` | 3               | If mode = 2, it changes gesture period<br>Reboot of the device is necessary |
| 0x24 | getSensorParameters | -                                | -               | Get sensor parameters (ALS Array, ALS Period, and gesture Period) |

**ActiveLook to Master**

| ID   | commands    | data      | Data length (B) | Description                   |
|------|-------------|-----------|-----------------|-------------------------------|
| 0x24 | getSensorParameters  | `u16 alsLum[9]`<br>`u16 alsPeriod`<br>`u16 gesturePeriod` | 25 | `alsLum`: ALS array to compare when luma is changed<br>`alsPeriod`: ALS period<br>`gesturePeriod`: gesture sensor period |
...
| ID   | commands    | Parameters                             | Data length (B)         | Description                                                     |
|------|-------------|----------------------------------------|-------------------------|-----------------------------------------------------------------|
| 0x41 | imgSave     | `u8 id`<br>`u32 size`<br>`u16 width`   | 7B for the first chunk  | Save 4bpp image of `size` bytes and `width` pixels              |
...
| 0x44 | imgStream   | `u32 size`<br>`u16 width`<br>`s16 x`<br>`s16 y` | 10B for the first chunk | Stream 1bpp bitmap image on display without saving it in memory |
| 0x45 | imgSave1bpp | `u32 size`<br>`u16 width`              | 6B for the first chunk  | Save 1bpp bitmap of `size` bytes and `width` pixels             |
...
| ID   | commands | data                        | Data length (B) | Description                                                  |
|------|----------|-----------------------------|-----------------|--------------------------------------------------------------|
| 0x47 | imgList  | `u8 id`<br>`u16 height`<br>`u16 width`<br>...<br>`u8 idN`<br>`u16 widthN`<br>`u16 heightN` | 5 x n           | list of images in memory<br>`height`and `width` are in pixel<br>Listing is not sorted by id |
...
| 0x51 | fontSave      | 1st chunk: `u8 id` `u16 size`<br>Then: `u8 data[n]` | 3B for the first chunk  | Save font `id` of `size` Bytes                               |
...
| ID   | commands | data                                    | Data length (B) | Description                                         |
|------|----------|-----------------------------------------|-----------------|-----------------------------------------------------|
| 0x50 | fontList | `u8 id`<br>`u8 height`<br>...<br>`u8 idN`<br>`u8 heightN` | 2 x n           | list of font in memory with there height            |
...
| 0x65 | layoutPosition        | `u8 id`<br>`u16 x`<br>`u8 y`  | 4                  | Redefine the position of a layout<br>Position is saved  |
| 0x66 | layoutDisplayExtended | `u8 id`<br>`u16 x`<br>`u8 y`<br>`str text[255]` | 5   | Display `text` with layout `id` at position `x` `y`<br>Position is not saved |
...
| ID   | commands   | data                           | Data length (B) | Description                                         |
|------|------------|--------------------------------|-----------------|-----------------------------------------------------|
| 0x64 | layoutList | `u8 id`<br>...<br>`u8 idN`     | n               | list of layouts in memory<br>Listing is not sorted by id |
...
| ID   | commands   | data                           | Data length (B) | Description                                         |
|------|------------|--------------------------------|-----------------|-----------------------------------------------------|
| 0x73 | gaugeList  | `u8 id`<br>...<br>`u8 idN`     | n               | list of gauges in memory<br>Listing is not sorted by id |
...
| 0x85 | pageList  | `u8 id`<br>...<br>`u8 idN`                                 | n               | List of page IDs in memory<br>Listing is not sorted by id |


###  4.12. <a name='Statisticscommands'></a>Statistics commands
...
| 0xA6 | setMaxPixelCnt     | `u32 maxPixelCnt`        | 4               | Set the maximum number of pixels activated simultaneously<br>0xFFFFFFFF disable the feature<br>The checker run periodically and clear screen if the number of pixels is reached |
| 0xA7 | getChargingCounter | -                        | 0               | Get total number of battery charging cycle              |
| 0xA8 | getChargingTime    | -                        | 0               | Get total number of battery charging minute             |
| 0xA9 | getMaxPixelCnt     | -                        | 0               | Get the maximum number of pixels activated simultaneously |
...
| ID   | commands           | data        | Data length (B) | Description                                         |
|------|--------------------|-------------|-----------------|-----------------------------------------------------|
| 0xA5 | pixelCount         | `u32 count` | 4               | Number of pixel activated on display                |
...
| 0xA9 | getMaxPixelCnt     | `u32 count` | 4               | Maximum number of pixels activated simultaneously   |

###  4.13. <a name='Configurationcommands'></a>Configuration commands
...
| ID   | commands        | Parameters                                  | Data length (B) | Description                                                       |
|------|-----------------|---------------------------------------------|-----------------|-------------------------------------------------------------------|
| 0xD0 | cfgWrite        | `str name[12]`<br>`u32 version`<br>`u32 password` | 10 <= n <= 20   | Write configuration, configurations are associated with layouts, images, etc<br>`name`: name of the configuration<br>`version`: provided by user, used to track versions of configuration<br>`password`: if configuration already exist, the same passsword must be provided as  the one used during the creation |
| 0xD1 | cfgRead         | `str name[12]`                              | 2 <= n <= 12    | Get number of elements stored in the configuration                |
...
###  4.14. <a name='Deprecatedcommands'></a>Deprecated commands


| ID   | commands    | Parameters                               | Data length (B)         | Description                                                     |
|------|-------------|------------------------------------------|-------------------------|-----------------------------------------------------------------|
| 0x40 | imgList     | -                                        | 0                       | Give the list of saved images<br>Deprecated since 4.0.0          |
| 0x41 | imgSave     | `u32 size`<br>`u16 width`                | 6B for the first chunk  | Save 4bpp image of `size` bytes and `width` pixels<br>Parameter `id` was added in 4.0.0<br>it's still possible to send the command without `id` if the new version with `id` was never used |
...
| ID   | commands  | data                        | Data length (B) | Description                                                  |
|------|-----------|-----------------------------|-----------------|--------------------------------------------------------------|
| 0x40 | imgList   | `u16 height`<br>`u16 width`<br>...<br>`u16 widthN`<br>`u16 heightN` | 4 x n           | list of images in memory<br>`height`and `width` are in pixel<br>Prior to 4.0.0, image listing wasn't giving the image id<br>Deprecated since 4.0.0 |
...
All graphical elements saved (layouts, images, etc) are regrouped in a configuration. Each application can use its own configuration. Multiple configurations can be stored in the device allowing fast switching between applications.

Configurations share a memory pool of 3MB. The device has one 'System config' that can't be deleted and multiple 'user config'. 'System config' is set during manufacturing and store 'System layouts' used for specific events (see Layout section).  

Before adding a new element, the user must ensure if there is enough memory available. This can be done with the `cfgFreeSpace` command. The maximum number of configurations is limites to 22.

On missing free space, some configurations must be deleted. Configurations can be listed with `cfgList` command to get their size, usage order (`usgCnt`), and install/update order (`installCnt`).  
The 'install counter' is updated on `cfgWrite` command, the 'usage counter' is updated on `cfgSelect` command  
...
It's tricky to know the size required to store an element. It doesn't only depend on the size of the element but also the memory fragmentation and the numbers of elements per sector.  
...
⚠ The `cfgWrite` command is required before any image upload.
...
Data are sent in chunks of 512 bytes with command `imgSave`  
...
## write config "Demo"
0xFFD0001244656D6F00000000000001E240AA
## save image #10, 20 bytes, 15 pixel width
...
It is also possible to stream images on display without saving them in memory with command `imgStream`. 
...
###  5.8. <a name='Font'></a>Font 

⚠ The `cfgWrite` command is required before any font upload.  
...
| Id | Name                       | Bold | Italic | Height [pxl] |
|----|----------------------------|------|--------|--------------|
| 0  | Computer Modern Sans Serif | no   | no     | 24           |
| 1  | Computer Modern Sans Serif | no   | no     | 24           |
| 2  | Computer Modern Sans Serif | no   | no     | 35           |
| 3  | Computer Modern Sans Serif | no   | no     | 49           |
...
⚠ The `cfgWrite` command is required before any layout upload.
...
|-----------|--------------------|---------------------------------------------------------------|
|     0     | Device boot        | Booting screen displayed for 5s                               |
|     1     | -                  | Reserved                                                      |
...
|      16     | bool      | Text opacity  | Define the argument opacity(on/off)                                              |
|    17…127   | u8 [size] | Commands      | Additional graphical commands                                                    |
...
####  5.9.1. <a name='LayoutPosition'></a>Layout Position
...
####  5.9.2. <a name='Defaultconfiguration'></a>Default configuration

A set of predefined layouts is present in the ActiveLook® device by default in the system configuration, these layouts use a set of default bitmap and font and are used by the ActiveLook® smart application to display information. This profile can be modified or updated  by the user.

| Layout ID | Name                |                                            command                                             |
|-----------|---------------------|:----------------------------------------------------------------------------------------------:|
...
⚠ The `cfgWrite` command is required before any page upload.
...
##  6. <a name='UIdesign:Goodpractices'></a>UI design: Good practices

In order to best leverage on the ActiveLook® device, here are a few guidelines to:
...
In order to accomodate the different personal face features to the optical eye box, it is better not to use the whole display area and keep "margins" on all sides. For instance, in the ActiveLook application we use a "protection" area of 30 pixels on either horizontal side and 25 pixels on either vertical sides. This makes an effective useful area of 244 x 206 pixels (See "Adjust screen offset" in the "settings" section and image below)  
The `shift` command (see above) is usefull to manage the visual margins.  
...
To ensure the transfer of each command, it is recommended to use the Flow Control server, as described in section [Flow Control server](#FlowControlserver).
